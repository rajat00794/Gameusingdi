...
class GamePlay:
    ...
    def start(self):
        if self.ready():
            self.player.player_name = input("enter player1 name:")
            self.player2.player_name = input("enter player2 name:")
            ...
            if hasattr(self.skill,self.player.strike.lower()):
                getattr(self.skill,self.player.strike.lower())(self.player,self.player2)
            if hasattr(self.skill,self.player2.strike.lower()):
                getattr(self.skill,self.player2.strike.lower())(self.player,self.player2)
